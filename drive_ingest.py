...
def get_chunks():
    if os.path.exists(CACHE_FILE):
        ...
        documents = ingest_drive_data()
        splitted_doc = split.split_documents(documents)
        with open(CACHE_FILE, 'wb') as f:
            pickle.dump(splitted_doc, f)
...
def process_all_chunks(splitted_doc):
    ...
    for i, chunk in enumerate(tqdm(splitted_doc)): # loop thru every doc in spplitted_doc
        try:
            extraction = call_ollama_model(chunk.page_content) #passes chunk into call ollama function
...
def push_to_google_doc(extracted_results):
    creds = get_api_service()
    service = build('docs', 'v1', credentials=creds)

    topics_dict = {
        "Culture": [], "Politics": [], "Education": [], "Environment": [],
        "Technology": [], "Economics": [], "Society": [], "Race/Religion": [],
        "Media": [], "Morality/Ethics": [], "General": []
    }
    ...
    for item in extracted_results:
        ...
        for part in sub_parts:
            ...
            for line in part.splitlines():
                ...
                if any(clean_line.lower().startswith(prefix) for prefix in ["topic:", "title:", "description:", "context:"]):
...
def call_ollama_model(sample_chunk):
    prompt = f"""
...
Classify the example into ONE of these primary topics: [Culture, Politics, Education, Environment, Technology, Economics, Society].

Format your output EXACTLY as follows, with no extra intro/outro text:

Topic: <Topic Name>
Title: <Short, Professional Example Title under 5 words>
Context: <2-3 sentences explaining the background, context, and key facts/stats (e.g. specific policies, historical background, figures, or geographical scope)>
Case Study / Concrete Detail: <3-7 sentences detailing the specific actions taken, specific actors involved (e.g., specific leaders, policies, organizations), mechanisms behind the issue>
...
"""
    ...
    response = ollama.generate(
        ...
        prompt=prompt,
        options={
            "num_predict": 800,  # Increased length to allow richer paragraphs
            "temperature": 0.2
        }
    )
...
if __name__ == '__main__':
    ...
    splitted_doc = get_chunks()
    print("Total number of chunks: ", len(splitted_doc))

    # 2. RE-RUN OLLAMA to generate fresh extractions using the updated prompt
    ...
    push_to_google_doc(extracted_results)
